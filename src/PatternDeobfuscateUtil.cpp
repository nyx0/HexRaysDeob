...
bool ExtractByOpcodeType(minsn_t *ins, mcode_t mc, minsn_t *&match,
                         mop_t *&noMatch) {
...
bool TunnelThroughAnd1(minsn_t *ins, minsn_t *&inner, bool bRequireSize1,
                       mop_t **opInner) {
...
// was of mop_d type).
bool ExtractLogicallyNegatedTerm(minsn_t *ins, minsn_t *&insNegated,
                                 mop_t **opNegated) {
  mop_t *nonNegated;
...
bool XorSimplifier::DidSimplify() {
  ...
  // return m_Const.size() != m_InsertedConst || m_NonConst.size() !=
  // m_InsertedNonConst;
}

...
struct AllocaFixer : minsn_visitor_t {
  ...
  int visit_minsn(void) {
    ...
    if (curins->opcode != m_call || curins->l.t != mop_h ||
        qstrcmp(curins->l.helper, "alloca") != 0)
      return 0;
    ...
    // Everything went according to plan. Save the call to alloca's address and
    // integer parameter.
    m_FixupLocations.push_back(
        std::pair<ea_t, int>(curins->ea, args[0].nnn->value));
    return 0;
  }
};
...
void FixCallsToAllocaProbe() {
  ...
  while (bFirst ? xr.first_to(eaAlloca, XREF_FAR) : xr.next_to()) {
    ...
    if (f == NULL) {
      msg("[E] Call to alloca from %a is not within a function; will "
          "not be processed\n",
          (uint64_t)xr.from);
      continue;
    }
  ...
  for (auto f : funcsCallingAlloca) {
    ...
    if (mba == NULL) {
      msg("[E] FixCallsToAllocaProbe(%a): decompilation failed (%s)\n",
          (uint64_t)f->start_ea, hf.desc().c_str());
      continue;
    }
    // Extract the alloca information by visiting its top-level instructions
    AllocaFixer af;
    mba->for_all_insns(af);
    // We own the mbl_array_t produced by gen_microcode, so we have to delete
    // it.
    delete mba;

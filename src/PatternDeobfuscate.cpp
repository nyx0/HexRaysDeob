...
struct ObfCompilerOptimizer : public optinsn_t {
  ...
  int pat_OrViaXorAnd(minsn_t *ins) {
    ...
    if (!(equal_mops_ignore_size(*xorOp1, *andOp1) &&
          equal_mops_ignore_size(*xorOp2, *andOp2)) ||
        (equal_mops_ignore_size(*xorOp1, *andOp2) &&
         equal_mops_ignore_size(*xorOp2, *andOp1)))
      return 0;
  ...
  bool NonConstSetsMatch(std::set<mop_t *> *s1, std::set<mop_t *> *s2) {
    ...
    for (auto eL : *s1) {
      ...
      // If we can't find some element from the first set in the other, we're
      // done
      if (!bFound)
        return false;
    }
  ...
  mop_t *FindNonCommonConstant(std::set<mop_t *> *smaller,
                               std::set<mop_t *> *bigger) {
    mop_t *noMatch = NULL;
  ...
  int pat_AndXor(minsn_t *ins) {
    ...
    else if (xsR.m_Const.size() == xsL.m_Const.size() + 1)
    ...
    noMatch->nnn->update_value(~noMatch->nnn->value &
                               ((1ULL << (noMatch->size * 8)) - 1));
  ...
  int pat_LnotOrLnotLnot(minsn_t *ins) {
    ...
    mop_t *opLeft, *opRight;
    if (!ExtractLogicallyNegatedTerm(inner->l.d, insLeft, &opLeft) ||
        !ExtractLogicallyNegatedTerm(inner->r.d, insRight, &opRight))
      return 0;
...
int ObfCompilerOptimizer::func(mblock_t *blk, minsn_t *ins, int optflags) {
  ...
  if ((is_mcode_jcond(ins->opcode) || is_mcode_set(ins->opcode)) &&
    ...
    struct Blah : minsn_visitor_t {
      int visit_minsn() { return othis->Optimize(this->curins); }
...
#endif
    ins->optimize_solo();
    // I got an INTERR if I optimized jcc conditionals without marking the
    // lists dirty.

...
struct mblock_qstring_dumper_t : public mblock_virtual_dumper_t {
  ...
  virtual void AddLine(qstring &qs) { qStr.append(qs); }
};

struct mblock_dumper_t : public mblock_virtual_dumper_t {
  strvec_t lines;
  mblock_dumper_t() : mblock_virtual_dumper_t() {};
  virtual void AddLine(qstring &qs) { lines.push_back(simpleline_t(qs)); }
};
...
protected:
  ...
  int GetIncrBlockNum() { return m_NumBlocks++; }
...
public:
  ...
  bool Display(minsn_t *top, sample_info_t *si, int nBlock, int nSerial) {
    ...
    m_Title.cat_sprnt("Microinstruction Graph - %a[%s]/%d:%d",
                      (uint64_t)mba->entry_ea, MicroMaturityToString(si->mat),
                      nBlock, nSerial);
    m_TW = create_empty_widget(m_Title.c_str());
    netnode id;
    id.create();

    m_GVName.cat_sprnt("microins_%a_%s_%d_%d", (uint64_t)mba->entry_ea,
                       MicroMaturityToString(si->mat), nBlock, nSerial);
    m_GV =
        create_graph_viewer(m_GVName.c_str(), id, migr_callback, this, 0, m_TW);
...
static ssize_t idaapi migr_callback(void *ud, int code, va_list va) {
  MicrocodeInstructionGraphContainer *gcont =
      (MicrocodeInstructionGraphContainer *)ud;
  MicrocodeInstructionGraph *microg = &gcont->m_MG;
  bool result = false;

  switch (code) {
  case grcode_reserved2:
    result = true;
    break;
...
public:
  ...
  bool Display(sample_info_t *si) {
    mbl_array_t *mba = *si->mba;
    m_Title.cat_sprnt("Microcode Graph - %a[%s]", (uint64_t)mba->entry_ea,
                      MicroMaturityToString(si->mat));

    TWidget *tw = create_empty_widget(m_Title.c_str());
    netnode id;
    id.create();

    m_GVName.cat_sprnt("microblkgraph_%a_%s", (uint64_t)mba->entry_ea,
                       MicroMaturityToString(si->mat));
    graph_viewer_t *gv =
        create_graph_viewer(m_GVName.c_str(), id, mgr_callback, this, 0, tw);
...
static bool idaapi ct_keyboard(TWidget * /*v*/, int key, int shift, void *ud) {
  if (shift == 0) {
    ...
    case 'I': {
      ...
      if (pDot == NULL) {
        warning("Couldn't find the block number on the current line; "
                "was the block empty?\n"
                "If it was not empty, and you don't see [int].[int] "
                "at the beginning of the lines\n"
                "please run the plugin again to generate a new "
                "microcode listing.\n"
                "That should fix it.");
      ...
      if (nBlock > mba->qty) {
        warning("Plugin error: line prefix was %d:%d, but block only "
                "has %d basic blocks.",
                nBlock, nSerial, mba->qty);
        return true;
      }
      ...
      if (minsn == NULL) {
        if (i == 0)
          warning("Couldn't get first minsn_t from %d:%d; was the "
                  "block empty?\n"
                  "If it was not empty, and you don't see "
                  "[int].[int] at the beginning of the lines\n"
                  "please run the plugin again to generate a new "
                  "microcode listing.\n"
                  "That should fix it.",
                  nBlock, nSerial);
        else
          warning("Couldn't get first minsn_t from %d:%d; last valid "
                  "instruction was %d",
                  nBlock, nSerial, i - 1);
        return true;
      }
      ...
      MicrocodeInstructionGraphContainer *mcg =
          new MicrocodeInstructionGraphContainer;
...
static const custom_viewer_handlers_t handlers(ct_keyboard,
                                               NULL, // popup
                                               NULL, // mouse_moved
                                               NULL, // click
                                               NULL, // dblclick
...
mba_maturity_t AskDesiredMaturity() {
  const char dlgText[] = "Select maturity level\n"
...
void ShowMicrocodeExplorer() {
  ...
  if (mba == NULL) {
    warning("#error \"%a: %s", (uint64_t)hf.errea, hf.desc().c_str());
    return;
  }
  ...
  qstring title;
  title.cat_sprnt("Microcode Explorer - %a - %s", (uint64_t)pfn->start_ea,
                  MicroMaturityToString(mmat));

  si->cv = create_custom_viewer(title.c_str(), // title
                                &s1,           // minplace
                                &s2,           // maxplace
                                &s1,           // curplace

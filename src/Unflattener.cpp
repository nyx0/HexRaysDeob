// ROLF TODO: Change control flow in func so that PruneUnreachable gets called
// even if CFI info fails? Probably doesn't matter at that point.
...
mblock_t *CFUnflattener::GetDominatedClusterHead(mbl_array_t *mba,
                                                 int iDispPred,
                                                 int &iClusterHead) {
...
int CFUnflattener::FindBlockTargetOrLastCopy(mblock_t *mb,
  ...
  bool bFound = FindNumericDefBackwards(mb, what, opNum, local, true,
                                        bAllowMultiSuccs, iClusterHead);
  ...
  m_DeferredErasuresLocal.insert(m_DeferredErasuresLocal.end(), local.begin(),
                                 local.end());
...
bool CFUnflattener::HandleTwoPreds(mblock_t *mb, mblock_t *mbClusterHead,
                                   mop_t *opCopy, mblock_t *&nonJcc,
                                   int &actualGotoTarget,
                                   int &actualJccTarget) {
  char buf[1000];
  mbl_array_t *mba = mb->mba;
  ...
  // nonJcc).
  if (!SplitMblocksByJccEnding(pred1, pred2, endsWithJcc, nonJcc, jccDest,
                               jccFallthrough)) {
    debugmsg("[I] Block %d w/preds %d, %d did not have one predecessor "
             "ending in jcc, one without\n",
  ...
  if (nonJcc->npred() != 1) {
    debugmsg("[I] Block %d w/preds %d, %d, non-jcc pred %d had %d "
             "predecessors (not 1)\n",
  ...
  if (nonJcc->pred(0) != endsWithJcc->serial) {
    debugmsg("[I] Block %d w/preds %d, %d, non-jcc pred %d did not have "
             "the other as its predecessor\n",
             iDispPred, pred1->serial, pred2->serial, nonJcc->serial);
    return false;
  }
  ...
  // not taken.
  actualGotoTarget =
      FindBlockTargetOrLastCopy(endsWithJcc, mbClusterHead, opCopy, false);

  // If that succeeded...
  if (actualGotoTarget >= 0) {
    // ... then do the same thing when the jcc is not taken.
    actualJccTarget =
        FindBlockTargetOrLastCopy(nonJcc, mbClusterHead, opCopy, true);
...
void CFUnflattener::ProcessErasures(mbl_array_t *mba) {
  m_PerformedErasuresGlobal.insert(m_PerformedErasuresGlobal.end(),
                                   m_DeferredErasuresLocal.begin(),
                                   m_DeferredErasuresLocal.end());
...
int idaapi CFUnflattener::func(mblock_t *blk) {
  ...
  snprintf(buf, sizeof(buf), "c:\\temp\\dumpBefore-%s-%d.txt", matStr,
           atThisMaturity);
...
#if UNFLATTENDEBUG
  snprintf(buf, sizeof(buf), "c:\\temp\\dumpAfter-%s-%d.txt", matStr,
           atThisMaturity);
  DumpMBAToFile(mba, buf);
#endif
  ...
  for (auto iDispPred : mba->get_mblock(cfi.iDispatch)->predset) {
    ...
    int iClusterHead;
    mblock_t *mbClusterHead =
        GetDominatedClusterHead(mba, iDispPred, iClusterHead);
    ...
    int iDestNo =
        FindBlockTargetOrLastCopy(mb, mbClusterHead, cfi.opAssigned, false);
...
#if UNFLATTENVERBOSE
    debugmsg("[I] Block %d did not define assign a number to assigned var; "
             "assigned %s instead\n",
    ...
    if (mb->npred() != 2) {
#if UNFLATTENVERBOSE
      debugmsg("[I] Block %d that assigned non-numeric value had %d "
               "predecessors, not 2\n",
               iDispPred, mb->npred());
#endif
      continue;
    ...
    // if-statement).
    if (HandleTwoPreds(mb, mbClusterHead, opCopy, nonJcc, actualGotoTarget,
                       actualJccTarget)) {
      // If it succeeded...

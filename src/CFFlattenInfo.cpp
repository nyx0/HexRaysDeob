...
bool JZInfo::ShouldBlacklist() {
...
#if UNFLATTENVERBOSE
    debugmsg("[I] Blacklisting due to lack of JZ/JG comparisons (%d < "
             "minimum of %d)\n",
  ...
  debugmsg("[I] %d comparisons, %d numbers, %d bits, %d ones, %f entropy\n",
           nSeen, nums.size(), iNumBits, iNumOnes, fEntropy);
#endif
...
struct BlockInsnAssignNumberExtractor : public minsn_visitor_t {
  ...
  int visit_minsn() {
    ...
    m_SeenAssignments.push_back(
        std::pair<mop_t *, uint64>(&curins->d, curins->l.nnn->value));
    return 0;
  }
};
...
struct HandoffVarFinder : public minsn_visitor_t {
  ...
  HandoffVarFinder(mop_t *opMax,
                   std::vector<std::pair<mop_t *, uint64>> &assignments)
      : m_ComparisonVar(opMax), m_SeenAssignments(assignments) {};

  int visit_minsn(void) {
    // We want copies into our comparison variable
    if (curins->opcode != m_mov ||
        !equal_mops_ignore_size(curins->d, *m_ComparisonVar))
      return 0;
...
struct JZMapper : public minsn_visitor_t {
  ...
  JZMapper(mop_t *mc, mop_t *ma, int iFirst, std::map<uint64, int> &map,
           std::map<int, uint64> &map2)
      : m_CompareVar(mc), m_AssignVar(ma), m_DispatchBlockNo(iFirst),
        m_KeyToBlock(map), m_BlockToKey(map2) {};
  ...
  int visit_minsn(void) {
    ...
    if (!equal_mops_ignore_size(*m_CompareVar, curins->l)) {
      ...
      if (blk->serial != m_DispatchBlockNo ||
          !equal_mops_ignore_size(*m_AssignVar, curins->l))
        return 0;
    }
...
bool CFFlattenInfo::GetAssignedAndComparisonVariables(mblock_t *blk) {
...
#if UNFLATTENVERBOSE
      debugmsg("[E] Comparison var was copied from %d "
               "assigned-to-constant variables, not 1 as expected\n",
               hvf.m_SeenCopies.size());
      for (auto sc : hvf.m_SeenCopies)
        debugmsg("\t%s (%d copies)\n", mopt_t_to_string(sc.first->t),
                 sc.second);
#endif
      return false;
    }
  ...
  // variable
  JZMapper jzm(opCompared, localOpAssigned, iDispatch, m_KeyToBlock,
               m_BlockToKey);

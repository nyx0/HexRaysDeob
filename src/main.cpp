 ...
 *
 * This uses the new C++ plugin API, it was tested with >= IDA 8.4 but should
 * be IDA 7.5 compatible
 *
...
#error This code requires IDA SDK 8.4 or higher
#endif
...
/// This variable must be instantiated by the plugin. It is initialized by
/// init_hexrays_plugin().
...
struct deobfuscator_ctx_t : public plugmod_t {
  ...
  const action_desc_t enable_action = ACTION_DESC_LITERAL_PLUGMOD(
      ENABLE_ACTION_NAME, "Enable OLLVM deobfuscator", &enable_ah, this, NULL,
      "Enable OLLVM deobfuscator", -1);

  disable_ah_t disable_ah = disable_ah_t(*this);
  const action_desc_t disable_action = ACTION_DESC_LITERAL_PLUGMOD(
      DISABLE_ACTION_NAME, "Disable OLLVM deobfuscator", &disable_ah, this,
      NULL, "Disable OLLVM deobfuscator", -1);

  microcode_ah_t ah;
  const action_desc_t microcode_action = ACTION_DESC_LITERAL_PLUGMOD(
      MICROCODE_ACTION_NAME, "Start MicroCode Explorer", &ah, this, NULL,
      "Open the MicroCode Explorer", -1);

  allocprob_ah_t allocprobe_ah;
  const action_desc_t fixallocaprobe_action = ACTION_DESC_LITERAL_PLUGMOD(
      ALLOCPROBE_ACTION_NAME, "Fix Calls To AllocaProbe", &allocprobe_ah, this,
      NULL, "Fix Calls To AllocaProbe", -1);
...
deobfuscator_ctx_t::deobfuscator_ctx_t() {
  // we need to register each action and then attach to a menu item in the
  // callback the moment we register the items are available via shortcut if it
  // exists
...
bool idaapi deobfuscator_ctx_t::run(size_t) { return true; }
...
plugin_t PLUGIN = {
    ...
    comment,                   // long comment about the plugin it could appear
                               // in the status line or as a hint
